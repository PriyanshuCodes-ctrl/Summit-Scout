...
function routeCardHTML(trek) {
  ...
  return `<article class="route-card">
    <div class="route-card-media">
      ...
      <span class="route-badge">${trek.name === 'Rupin Pass' ? '★ Most Popular' : 'Explore'}</span>
    </div>
    <div class="route-card-body">
      ...
      <div class="route-stats">
        <div><b>▲ ${trek.alt.toLocaleString()} m</b><small>Max altitude</small></div>
        <div><b>⌁ ${trek.dist} km</b><small>Total distance</small></div>
        <div><b>${esc(trek.diff)}</b><small>Difficulty</small></div>
      </div>
      <a class="route-link ss-card-button" href="trek.html?trek=${encodeURIComponent(trek.name)}">Explore <span>→</span></a>
    </div>
  </article>`;
}
